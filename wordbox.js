...
function updateGame() {
	var game = `/kidjuice/wordbox/?game=${gameState.options.size}${gameState.unlimitedTime?'1':'0'}${gameState.seed}`;
	if (isMobile) {
		try {
	...
	if (navigator?.clipboard) {
		navigator.clipboard.writeText("http://cricketsongmedia.com"+game);
	}
}
